...
const photosAll=[];
const podEntries=[];
histFiltered.forEach(e=>{if((e.photos&&e.photos.length>0)||e.signature){const validPhotos=(e.photos||[]).filter(p=>!(typeof p==="string"&&p.startsWith("photo_")));const addr=e.addr||getAddr(e.stop);const drv=drivers.find(d=>d.id===e.driverId);podEntries.push({...e,addr,driverName:drv?.name||"",validPhotos});validPhotos.forEach((p,pi)=>photosAll.push({src:p,stop:e.stop,customer:e.customer,dayName:e.dayName,dayDate:e.dayDate,signature:e.signature,id:e.id+"-"+pi,addr,driverName:drv?.name||"",arrivedAt:e.arrivedAt,departedAt:e.departedAt,weight:e.weight,allPhotos:validPhotos,entryId:e.id}));}});
...
if((e.photos&&e.photos.length>0)||e.signature){
const validPhotos=(e.photos||[]).filter(p=>!(typeof p==="string"&&p.startsWith("photo_")));
const addr=e.addr||getAddr(e.stop);
const drv=drivers.find(d=>d.id===e.driverId);

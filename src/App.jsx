...
if(window._fbOps?.uploadFile){
  fetch(dataUrl).then(r=>r.blob()).then(b=>{
    ...
    console.log("[PHOTO-DRV] Uploading to Storage:",path);
    ...
    console.error("[PHOTO-DRV] ✗ Storage upload failed:",err?.code,err?.message);
    showToast("⚠ Photo save failed: "+( err?.code||err?.message||"unknown error"));
  });
}else{

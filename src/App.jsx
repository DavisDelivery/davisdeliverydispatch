...
const makeNote=(dels)=>{
  ...
  /* Reverse the delivery order to reflect LOAD order. Trucks load LIFO —
     the item delivered LAST needs to be loaded FIRST (deep in the truck);
     the item delivered FIRST needs to be loaded LAST (at the door). So
     the pickup list shows items in the order the driver should physically
     load them onto the truck at the pickup site. Prefix makes it explicit. */
  const names=dels.slice().reverse().map(e=>e.stop);
  const prefix="Load order: ";

...
const[savedQuotes,setSavedQuotes]=useState(()=>{const v=lsGet("dd_quotes",[]);const arr=Array.isArray(v)?v:Object.values(v||{});return arr.filter(q=>q&&typeof q==="object").map(q=>({...q,rate:typeof q.rate==="number"?q.rate:(parseFloat(q.rate)||0)}));});
...
useEffect(()=>{
  ...
  const unsubQuotes=subscribeQuotes((fbQuotes)=>{const arr=Array.isArray(fbQuotes)?fbQuotes:Object.values(fbQuotes||{});const clean=arr.filter(q=>q&&typeof q==="object").map(q=>({...q,rate:typeof q.rate==="number"?q.rate:(parseFloat(q.rate)||0)}));setSavedQuotes(clean);});
...
useEffect(()=>{
  ...
  const poll=async()=>{
    try{
      ...
      console.log(`[MOTIVE] Poll: ${data.vehicles.length} vehicles, ${matched} tracked, ${skipped} not enabled, ${unmatched} no match`);

...
const{totalMins:shiftMins}=getShiftSummary(emDk);
/* ── Delivery counter ── */
const allDels=dl.filter(e=>e.stopType!=="pickup");
const totalDels=allDels.length;
const doneDels=allDels.filter(e=>e.status==="departed").length;
const unassignedDels=allDels.filter(e=>e.driverId===0).length;
const driverDelCounts=drivers.map(drv=>{const n=allDels.filter(e=>e.driverId===drv.id).length;return n>0?`${drv.name.split(" ")[0]}: <b>${n}</b>`:null;}).filter(Boolean).join(" &nbsp;·&nbsp; ");
h+=`<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px;margin-bottom:10px;padding:10px 16px;background:#f0f5fa;border-radius:10px;border:1.5px solid #bfdbfe">`;
h+=`<div style="display:flex;align-items:baseline;gap:10px;flex-wrap:wrap">`;
h+=`<span style="font-size:22px;font-weight:800;color:#1e5b92">${totalDels}</span>`;
h+=`<span style="font-size:13px;font-weight:700;color:#1e5b92">Total Deliveries</span>`;
if(doneDels>0)h+=`<span style="font-size:11px;color:#16a34a;font-weight:700;background:#f0fdf4;padding:2px 8px;border-radius:4px;border:1px solid #bbf7d0">\u2713 ${doneDels} done</span>`;
if(unassignedDels>0)h+=`<span style="font-size:11px;color:#dc2626;font-weight:700;background:#fef2f2;padding:2px 8px;border-radius:4px;border:1px solid #fca5a5">\u26a0 ${unassignedDels} unassigned</span>`;
h+=`</div>`;
if(driverDelCounts)h+=`<div style="font-size:11px;color:#57534e">${driverDelCounts}</div>`;
h+=`</div>`;

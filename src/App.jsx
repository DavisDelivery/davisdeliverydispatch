...
{/* Route line toggle */}
<button onClick={()=>setUseRoadRoutes(!useRoadRoutes)} style={{position:"absolute",bottom:activeDriver&&onAssignStop?52:12,left:12,zIndex:5,background:useRoadRoutes?"#1e5b92":"#fff",color:useRoadRoutes?"#fff":"#57534e",border:"1px solid "+(useRoadRoutes?"#1e5b92":"#d6d3d1"),borderRadius:8,padding:"6px 10px",fontSize:10,fontWeight:700,cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.15)",display:"flex",alignItems:"center",gap:4}}>
...
<select value={entry.driverId} onChange={e=>{e.stopPropagation();const v=e.target.value;if(v.includes("-")){const p=v.split("-");reassign(entry.id,Number(p[0]));setLoadNum(entry.id,Number(p[1]));}else{reassign(entry.id,Number(v));}}} style={{background:"#f5f5f4",border:"1px solid #e7e5e4",borderRadius:5,padding:"2px 4px",fontSize:9,color:"#57534e",cursor:"pointer",maxWidth:80}}><option value={0}>{entry.driverId>0?"↩ Unassign":"Assign"}</option>{drivers.flatMap(dd=>{const nl=getDriverLoadOptions(dd.id);const opts=[<option key={dd.id} value={dd.id}>{dd.name}</option>];for(let ln=2;ln<=nl;ln++)opts.push(<option key={dd.id+"-"+ln} value={dd.id+"-"+ln}>{dd.name.split(" ").map(w=>w[0]).join("")+" L"+ln}</option>);return opts;})}</select>

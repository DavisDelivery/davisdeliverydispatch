...
const allDriverEntries=drivers.map((drv,di)=>({drv,di,entries:drvEntries(drv.id)})).filter(({drv})=>drv.active!==false||driverHasWeekEntries(drv.id));
...
{drivers.map((drv,di)=>({drv,di})).filter(({drv})=>drv.active!==false||driverHasWeekEntries(drv.id)).map(({drv,di})=>{const de=drvEntries(drv.id);const isDrvDropTarget=dragSrc&&dragSrc.drvId!==drv.id&&dragOver?.drvId===drv.id;return(
<div key={drv.id}

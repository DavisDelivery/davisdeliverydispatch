...
const mi=parseFloat(miles)||0;const baseTier=getBaseTier(mi);const fuelAmt=(liftgate||noFuel)?0:baseTier*0.15;const total=baseTier+fuelAmt+(liftgate?75:0)+(gravel?25:0)+(extraPallets?25:0);const isOneOff=customerName==="One-Off Delivery";
...
let fuel=0;
/* noFuel overrides everything — $0 fuel surcharge.
   Liftgate is still flagged for operational awareness, but its $75 fee
   (which normally *replaces* the fuel surcharge) is also waived in no-fuel mode. */
if(!noFuel){
  if(liftgate){fuel=75;}else{fuel=Math.round(base*effRate);}
}
if(gravel)base+=25;
...
{qMiles>0&&!qRate&&<div style={{fontSize:12,fontWeight:700,color:"#16a34a",marginTop:6,textAlign:"center"}}>Auto Rate: {fmt(calcQuoteRate(qMiles,qLiftgate,qGravel,qExtraPallets,qNoFuel).total)} ({fmt(calcQuoteRate(qMiles,qLiftgate,qGravel,qExtraPallets,qNoFuel).base)} base{qNoFuel?" · no fuel":" + "+fmt(calcQuoteRate(qMiles,qLiftgate,qGravel,qExtraPallets,qNoFuel).fuel)+" "+(qLiftgate?"liftgate":"fuel")})</div>}
</div>
...
{customDelMiles>0&&(()=>{const _fr=CUSTOMERS[selCust]?.fuel_surcharge&&!CUSTOMERS[selCust]?.fuel_included?CUSTOMERS[selCust].fuel_surcharge:0.15;const calc=calcQuoteRate(customDelMiles,customDelLiftgate,customDelGravel,customDelExtraPallets,customDelNoFuel,_fr);const fuelLabel=customDelNoFuel?"no fuel":customDelLiftgate?"liftgate":Math.round(_fr*100)+"% fuel";return<div style={{fontSize:11,fontWeight:700,color:"#16a34a",textAlign:"center",marginTop:6}}>Auto: {fmt(calc.total)} ({fmt(calc.base)} base{customDelNoFuel?" · no fuel":" + "+fmt(calc.fuel)+" "+fuelLabel})</div>;})()}
</div>}

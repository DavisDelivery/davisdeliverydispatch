...
/* Load 2+ uses a shifted color and dashed line to visually separate from Load 1 */
...
const entry=dl.find(e=>e.id===eid);
if(!entry)return;
/* Find all siblings at the same stop (same stop name + address) */
...
/* If clicked stop is already assigned to this driver, remove all siblings */
...
/* Single atomic update: reassign + reorder to end of driver's stops */
setLog(p=>{
  let all=[...(p[dk]||[])];
  /* Mark all siblings as assigned to this driver */
  all=all.map(e=>sibIds.includes(e.id)?{...e,driverId:did,loadNum:loadNum||e.loadNum||1}:e);
  /* Move each sibling to end of this driver's stops */
  sibIds.forEach(sid=>{
  ...
  /* Rebuild pickups for delivery stops */
  sibIds.forEach(sid=>{
    const ent=all.find(e=>e.id===sid);
...
const printAllManifests=()=>{printContent(`All Manifests — ${wd[sd].name} ${wd[sd].date}`,()=>{
...
const DCOL_PRINT=["#2563eb","#16a34a","#ea580c","#9333ea"];
drivers.forEach((drv,di)=>{
  ...
  const col=DCOL_PRINT[di]||"#1e5b92";
  h+=`<div class="section" style="${di>0?"page-break-before:always":""}">`;
  h+=`<div class="section-title" style="border-left-color:${col};background:${col}15;padding:10px 14px">`;
  h+=`<div style="display:flex;align-items:center;gap:10"><div style="width:34px;height:34px;border-radius:8px;background:${col};display:flex;align-items:center;justify-content:center;color:#fff;font-size:17px;font-weight:800">${drv.name.charAt(0)}</div>`;
  h+=`<div><div style="font-size:16px;font-weight:800;color:${col}">${drv.name}</div><div style="font-size:11px;color:#78716c">${de.filter(e=>e.stopType!=="pickup").length} deliveries${drvMiles>0?" · ~"+drvMiles+"mi":""}${loads.length>1?" · "+loads.length+" loads":""}</div></div></div>`;
  h+=`<span style="color:#16a34a;font-weight:800;font-size:18px;font-variant-numeric:tabular-nums">${fmt(drvRev)}</span></div>`;
  loads.forEach(ln=>{
    ...
    h+=`<table><tr><th>#</th><th>Type</th><th>Stop</th><th>Address</th><th>Wt (lbs)</th><th>Notes</th><th style="text-align:right">Rate</th></tr>`;
    loadStops.forEach((e,i)=>{
      ...
      const lgBadge=e.liftgateApplied?'<span style="font-size:8px;background:#fef3c7;color:#92400e;padding:1px 4px;border-radius:2px;font-weight:700;margin-right:3px">LG</span>':"";
      const dueTag=e.dueBy?`<span style="font-size:8px;background:#dc2626;color:#fff;padding:1px 4px;border-radius:2px;font-weight:700;margin-right:3px">⏰ ${e.dueBy}</span>`:"";
      const pickupFrom=e.pickupFrom&&e.stopType!=="pickup"?`<div style="font-size:9px;color:#78716c">from ${e.pickupFrom}</div>`:"";
      const noteCell=`${dueTag}${lgBadge}${e.customer}${e.instructions?` | 📋 ${e.instructions}`:""}${e.note?` | ${e.note}`:""}`;
      h+=`<tr style="background:${e.stopType==="pickup"?"#eff6ff":i%2===0?"#fff":"#fafaf9"}">`;
      h+=`<td style="font-weight:700;color:#a8a29e;width:24px">${i+1}</td><td style="width:40px">${tag}</td>`;
      h+=`<td><b style="font-size:12px">${e.stop}</b>${pickupFrom}</td>`;
      h+=`<td style="font-size:10px;color:#78716c">${e.addr||""}</td>`;
      h+=`<td style="font-size:11px;font-weight:700;color:#1e5b92;white-space:nowrap;text-align:right">${e.weight>0?e.weight.toLocaleString():""}</td>`;
      h+=`<td style="font-size:10px;color:#57534e">${noteCell}</td>`;
      h+=`<td style="text-align:right;font-weight:800;white-space:nowrap;width:60px">${e.isHourly?"HR":fmt(e.baseRate)}</td></tr>`;
    });
    h+=`</table>`;
  });
  h+=`</div>`;
...
if(ua.length){h+=`<div class="section"><div class="section-title" style="border-left-color:#dc2626">⚠ Unassigned (${ua.length})</div><table><tr><th>#</th><th>Type</th><th>Stop</th><th>Customer</th><th>Address</th><th>Wt</th><th style="text-align:right">Rate</th></tr>`;ua.forEach((e,i)=>{const tag=e.stopType==="pickup"?'<span class="tag pu">PU</span>':e.priority?'<span class="tag pri">PRI</span>':'<span class="tag del">DEL</span>';h+=`<tr style="background:#fef2f2"><td>${i+1}</td><td>${tag}</td><td><b>${e.stop}</b></td><td>${e.customer}</td><td style="font-size:10px">${e.addr||""}</td><td style="font-size:10px">${e.weight>0?e.weight.toLocaleString():""}</td><td style="text-align:right;font-weight:700">${e.isHourly?"HR":fmt(e.baseRate)}</td></tr>`;});h+=`</table></div>`;}
return h;});};
...
setLog(p=>{
  let all=[...(p[dk]||[])];
  /* Step 1: apply all driver assignments from rpOrders */
  ...
  /* Step 2: mark unassigned stops as driverId=0 */
  const allRouted=Object.values(rpOrders).flat();
  all=all.map(e=>allRouted.includes(e.id)?e:{...e,driverId:0});
  /* Step 3: reorder each driver's stops per rpOrders sequence */
  Object.entries(rpOrders).forEach(([did,ids])=>{
  ...
  /* Step 4: rebuild pickup cards for every delivery stop that has a customer with pickups */

    ...
    if(window.google?.maps?.Geocoder){
      ...
      geocoder.geocode({address:addr},(results,status)=>{
        if(status==="OK"&&results[0]){
          ...
          /* Fallback: use Places API findPlaceFromQuery */
          const div=document.createElement("div");
          const svc=new window.google.maps.places.PlacesService(div);
          svc.findPlaceFromQuery({query:addr,fields:["geometry"]},(results2,status2)=>{
            if(status2==="OK"&&results2[0]?.geometry?.location){
              const loc2=results2[0].geometry.location;
              _dynamicCoords[addr]={lat:loc2.lat(),lng:loc2.lng()};
              _saveGeoCache();
              if(_geocodeNotify)_geocodeNotify();
            }
          });
      ...
      /* Geocoder not available, try Places API directly */
      const div=document.createElement("div");
      const svc=new window.google.maps.places.PlacesService(div);
      svc.findPlaceFromQuery({query:addr,fields:["geometry"]},(results,status)=>{
...
/* Known liftgate: keep base rate as-is, use liftgateFee for +$75, zero out fuel */
...
showToast(`${stop} added — LG $75 included, no fuel`);
...
<InlineRate value={entry.baseRate} isHourly={entry.isHourly} onSave={r=>updateRateForDay(entry.id,r,dayKey)}/>
</div>);})}
</div>);})}
...
const stop=ovr?ovr.s:origStop;const isStopLG=!isStr&&d.lg;const rate=isStopLG?((ovr?(ovr.r||0):origRate)+75):(ovr?(ovr.r||0):origRate);const note=ovr?(ovr.n||null):origNote;const addr=ovr?(ovr.addr||origAddr):origAddr;

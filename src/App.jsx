...
const mi=parseFloat(miles)||0;const baseTier=getBaseTier(mi);const fuelAmt=noFuel?0:Math.round(baseTier*0.15);const total=baseTier+fuelAmt+(liftgate?75:0)+(gravel?25:0)+(extraPallets?25:0);const isOneOff=customerName==="One-Off Delivery";
...
const addQuote=drvId=>{const stopName=customStop||customerName||(isOneOff?"One-Off":"Quote");const cust=isOneOff?"One-Off Delivery":"Quote Delivery";const noteItems=[`${mi}mi`,liftgate?"LG":(noFuel?"No Fuel":"Fuel"),gravel?"Gravel":"",extraPallets?"4-5 Pallets":""].filter(Boolean).join(" | ");
...
{!liftgate&&noFuel&&<div style={{padding:"10px 14px",background:"#ecfeff",borderRadius:10,border:"1px solid #a5f3fc",marginTop:6}}><span style={{fontSize:12,color:"#155e75",fontWeight:600}}>Fuel surcharge waived</span></div>}
</div>

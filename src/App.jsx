    ...
    if(status==="OK"){
      const renderer=new window.google.maps.DirectionsRenderer({map,directions:result,suppressMarkers:true,preserveViewport:true,
        polylineOptions:{strokeColor:col,strokeOpacity:0.8,strokeWeight:5,
          icons:[{icon:{path:'M 0,-1 L 2,0 L 0,1',scale:5,fillColor:col,fillOpacity:1,strokeWeight:0},offset:'100%',repeat:'60px'}]}});
...
{custDels.filter(s=>!s.startsWith("Transfer")&&s!=="Drop Ship Liftgate").map(s=>{const addr=getAddr(s);return(<button key={s} onClick={()=>{setPuCustomName(s);if(addr)setPuCustomAddr(addr);}} style={{display:"block",width:"100%",textAlign:"left",padding:"7px 10px",marginBottom:1,borderRadius:7,cursor:"pointer",fontSize:12,fontWeight:600,background:puCustomName===s?"#2563eb":"#fff",color:puCustomName===s?"#fff":"#1c1917",border:puCustomName===s?"1px solid #1d4ed8":"1px solid transparent"}}>{s}{addr?<span style={{fontSize:9,color:puCustomName===s?"#bfdbfe":"#a8a29e",marginLeft:6}}>{addr.split(",")[0]}</span>:""}</button>);})}
</div>

...
const calcRouteMiles=(entries)=>{
  ...
  const pts=[origin];
  entries.forEach(e=>{const c=_resolveStopCoords(e);if(c)pts.push(c);});
  let total=0;
...
const getAddr=(s)=>{
  if(ADDR[s])return ADDR[s];
  if(_customAddrCache[s])return _customAddrCache[s];
  /* Handle AI-generated full names: "Dalton Carpet One - Eatonton" → "DCO Eatonton" */
  const aliases=[["Dalton Carpet One","DCO"],["Dalton Carpet Outlet","DCO"]];
  for(const[full,short]of aliases){
    if(s.includes(full)){
      const suffix=s.replace(new RegExp("^.*?[-–—]\\s*"),"").trim();
      for(const k of Object.keys(ADDR)){if(k.startsWith(short)&&k.toLowerCase().includes(suffix.toLowerCase()))return ADDR[k];}
    }
  }
  return "";
};
/* Resolve best address+coords for a stop entry — single source of truth */
const _resolveStopCoords=(e)=>{
  if(!e)return null;
  const knownAddr=getAddr(e.stop);
  const addr=e.addr||knownAddr;
  /* Try the entry's addr first, then the known addr from dictionary */
  return getCoords(addr)||(knownAddr&&knownAddr!==addr?getCoords(knownAddr):null);
};
...
const _rbCoord=_resolveStopCoords;
...
const _sCoord=_resolveStopCoords;
...
const coords=dels.map(e=>_resolveStopCoords(e)).filter(Boolean);
...
const mapS=dl.map(e=>{const coords=_resolveStopCoords(e);if(!coords)return null;
...
{(()=>{
const stopsWithCoords2=dl.map(e=>{const coords=_resolveStopCoords(e);if(!coords)return null;

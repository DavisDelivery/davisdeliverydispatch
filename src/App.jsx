  ...
  svc.route({origin,destination:dest,waypoints,travelMode:window.google.maps.TravelMode.DRIVING,optimizeWaypoints:false},(result,status)=>{
    if(status==="OK"){
      const renderer=new window.google.maps.DirectionsRenderer({map,directions:result,suppressMarkers:true,preserveViewport:true,
        polylineOptions:{strokeColor:col,strokeOpacity:0.8,strokeWeight:5,
...
<select value={"0:1"} onChange={e=>{const v=e.target.value;if(v==="0:1")return;const[did,ln]=v.split(":").map(Number);reassign(entry.id,did);if(ln)setLoadNum(entry.id,ln);}} style={{background:"#f5f5f4",border:"1px solid #e7e5e4",borderRadius:5,padding:"3px 6px",fontSize:10,color:"#57534e",cursor:"pointer"}}>

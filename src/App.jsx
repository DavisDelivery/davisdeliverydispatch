...
const saveManifestDay=async(wo,sd,entries)=>{
  try{
    await _fbReady.catch((e)=>{console.error("[FB] Not ready:",e);});
    if(!_db()){console.error("[FB-SAVE] No DB — Firebase not loaded");return;}
    ...
    console.error("[FB-SAVE] FAILED manifests/"+wo+"-"+DAYNAMES_FB[sd],e);
  }
};
...
const saveMessage=async(channelKey,msg)=>{
  try{
    ...
    console.error("[MSG] Write FAILED:",e);
    throw e;
  }
};
...
useEffect(()=>{
  ...
  setFbConnected(true);
  console.log("[FB] Firebase connected, all subscriptions active");
  ...
  const unsubManifests=subscribeManifests(wo,(fbData)=>{
    setLog(prev=>{
      ...
      Object.entries(fbData).forEach(([dayKey,entries])=>{
        ...
        if(dayIdx<0)return;
        const lk=`${wo}-${dayIdx}`;
        /* Mark that we've heard from Firebase for this day */
        fbHeardRef.current[lk]=true;
        if(entries&&entries.length>0){
          /* Firebase has data — use it as truth */
          updated[lk]=entries;
          prevLogRef.current[lk]=JSON.stringify(entries);
          console.log("[SUB] Received",dayKey,"entries:",entries.length);
        }else{
          /* Firebase is empty — if local has data, push it to Firebase */
          const localEntries=prev[lk]||[];
          if(localEntries.length>0){
            console.log("[SUB] Firebase empty for",lk,"— pushing",localEntries.length,"local entries");
            prevLogRef.current[lk]=JSON.stringify(localEntries);
            saveManifestDay(wo,dayIdx,localEntries).catch(e=>console.error("Push:",e));
          }else{
            prevLogRef.current[lk]="[]";
          }
        }
      });
      return updated;
...
const fbHeardRef=useRef({}); /* tracks which days we've received Firebase data for */
useEffect(()=>{
  /* ALWAYS save to localStorage */
  lsSet(LS_LOG,log);
  /* Save to Firebase — but only days we've heard from Firebase about (prevents startup overwrite) */
  if(!firebaseReady.current)return;
  const timer=setTimeout(()=>{
    Object.keys(log).forEach(dayKey=>{
      /* Don't push to Firebase until we've received at least one snapshot for this day */
      if(!fbHeardRef.current[dayKey])return;
      ...
      if(cur!==prev){
        ...
        if(!isNaN(weekOff)&&dayIdx>=0&&dayIdx<=4){
          ...
          saveManifestDay(weekOff,dayIdx,log[dayKey]||[]).then(()=>console.log("[SAVE] OK",dayKey)).catch(e=>console.error("[SAVE] FAILED",dayKey,e));
        }
      }
    });
  },500);

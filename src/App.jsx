...
const mapS=dl.map(e=>{const knownAddr=getAddr(e.stop);const addr=e.addr||knownAddr;const coords=getCoords(addr)||(knownAddr&&knownAddr!==addr?getCoords(knownAddr):null);if(!coords)return null;
...
{(()=>{
const stopsWithCoords2=dl.map(e=>{const knownAddr=getAddr(e.stop);const addr=e.addr||knownAddr;const coords=getCoords(addr)||(knownAddr&&knownAddr!==addr?getCoords(knownAddr):null);if(!coords)return null;

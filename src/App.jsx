...
/* On-demand history loader. The two subscriptions above only cover wo and
   wo-1. Anything older has to be fetched explicitly when the user enters
   the History tab or expands the lookback range. We do one-time reads
   (not subscriptions) and write straight into the same `log` object the
   subscriptions populate, so the existing getHistoryEntries() iteration
   works unchanged.

   _rawSetLog (not setLog) so these reads don't mark days dirty and trigger
   a save loop. loadedHistWeeksRef de-duplicates repeated entries into the
   tab. Bounded at 26 weeks per pass to keep an "All" selection from
   stampeding Firestore. */
useEffect(()=>{
  if(view!=="history"||histMode!=="deliveries")return;
  if(!window._fbOps)return;
  const target=histWeekRange>=999?26:Math.min(histWeekRange,26);
  const weeksToLoad=[];
  for(let w=wo-2;w>=wo-target;w--){
    if(!loadedHistWeeksRef.current.has(w))weeksToLoad.push(w);
  }
  ...
  (async()=>{
    for(const w of weeksToLoad){
      if(cancelled)break;
      loadedHistWeeksRef.current.add(w); /* mark first to avoid duplicate concurrent loads */

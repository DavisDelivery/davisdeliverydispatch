...
<select value={entry.driverId} onChange={e=>{e.stopPropagation();const v=e.target.value;if(v.includes("-")){const[did,ln]=v.split("-").map(Number);reassign(entry.id,did);if(ln>0)setLoadNum(entry.id,ln);}else{reassign(entry.id,Number(v));}}} style={{background:"#f5f5f4",border:"1px solid #e7e5e4",borderRadius:5,padding:"2px 4px",fontSize:9,color:"#57534e",cursor:"pointer",maxWidth:80}}><option value={0}>Assign</option>{drivers.map(dd=>{const nl=getDriverLoadOptions(dd.id);return nl>1?[<option key={dd.id} value={dd.id} disabled style={{fontWeight:700}}>{dd.name}</option>,...Array.from({length:nl},(_,i)=>i+1).map(ln=><option key={dd.id+"-"+ln} value={dd.id+"-"+ln}>{"\u00A0\u00A0"+dd.name.split(" ").map(w=>w[0]).join("")+" L"+ln}</option>)]:[<option key={dd.id} value={dd.id}>{dd.name}</option>];}).flat()}</select>
...
<select value={0} onChange={e=>{const v=e.target.value;if(v.includes("-")){const[did,ln]=v.split("-").map(Number);reassign(entry.id,did);if(ln>0)setLoadNum(entry.id,ln);}else{reassign(entry.id,Number(v));}}} style={{background:"#f5f5f4",border:"1px solid #e7e5e4",borderRadius:5,padding:"3px 6px",fontSize:10,color:"#57534e",cursor:"pointer"}}>
<option value={0}>Assign...</option>{drivers.map(dd=>{const nl=getDriverLoadOptions(dd.id);return nl>1?[<option key={dd.id} value={dd.id} disabled style={{fontWeight:700}}>{dd.name}</option>,...Array.from({length:nl},(_,i)=>i+1).map(ln=><option key={dd.id+"-"+ln} value={dd.id+"-"+ln}>{"\u00A0\u00A0"+dd.name.split(" ").map(w=>w[0]).join("")+" L"+ln}</option>)]:[<option key={dd.id} value={dd.id}>{dd.name}</option>];}).flat()}
</select>

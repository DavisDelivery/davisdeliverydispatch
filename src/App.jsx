  ...
  try{
    ...
    for(let wOff=0;wOff>=-51;wOff--){
      ...
      weekEntries.forEach(e=>{
        const lineRate=e.baseRate+(e.knownLiftgate?(e.liftgateFee||0):0);
...
if(pickupName&&originAddr&&onAddQuote){
  onAddQuote(cust,{puStop:pickupName,puAddr:originAddr,puNote:`Picking up for ${stopName}`+(notes.trim()?" | "+notes.trim():"")},{delStop:stopName,delAddr:destAddr,delRate:total,delNote:"from "+pickupName+" | "+finalNote,pickupFrom:pickupName,delWeight:wt,delFuelPct:effFuelPct,delLiftgate:liftgate},drvId);
}else{
  onAdd(cust,stopName,total,drvId,{note:(pickupName?"from "+pickupName+" | ":"")+finalNote,addr:destAddr,stopType:"delivery",pickupFrom:pickupName||null,weight:wt,fuelPct:effFuelPct,liftgateApplied:liftgate,knownLiftgate:liftgate,liftgateFee:liftgate?75:0});
}
};
return(
<div>
...
addDel(selCust,puCustomName.trim(),0,drvId,{stopType:"pickup",addr:puCustomAddr.trim(),note:puNote.trim()||null,weight,manualPickup:true});
...
addDel(selCust,delStop,rate,drvId,{stopType:"delivery",addr:delAddr,note:(puNote.trim()?puNote.trim()+" | ":"")+(puMiles?puMiles+"mi":""),pickupFrom:puCustomName.trim(),weight,fuelPct:(cd.fuel_surcharge&&!cd.fuel_included)?cd.fuel_surcharge:0,isHourly:cd.rate_type==="hourly"&&!emserLTL,priority:cd.priority});

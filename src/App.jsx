...
if(pickupName&&originAddr&&onAddQuote){
  onAddQuote(cust,{puStop:pickupName,puAddr:originAddr,puNote:`Picking up for ${stopName}`+(notes.trim()?" | "+notes.trim():"")},{delStop:stopName,delAddr:destAddr,delRate:baseRateForManifest,delNote:"from "+pickupName+" | "+finalNote,pickupFrom:pickupName,delWeight:wt,delFuelPct:effFuelPct,delLiftgate:liftgate},drvId);
}else{
  onAdd(cust,stopName,baseRateForManifest,drvId,{note:(pickupName?"from "+pickupName+" | ":"")+finalNote,addr:destAddr,stopType:"delivery",pickupFrom:pickupName||null,weight:wt,fuelPct:effFuelPct,liftgateApplied:liftgate,knownLiftgate:false,liftgateFee:liftgate?75:0});
}
};
return(
<div>
...
{(()=>{const isPU=entry.stopType==="pickup";const isManualPU=isPU&&entry.manualPickup;const pf=entry.pickupFrom;const cust=entry.customer;if(!cust&&!pf)return null;/* Label & display vary by stop type:
   ...
   - Delivery w/o pickupFrom: 'Pickup from {customer}' */const label=isManualPU?"For":"Pickup from";const display=isPU?cust:((pf&&pf.includes(" - "))?pf:(cust+(pf?" — "+pf:"")));return(<><span style={{fontSize:10,color:"#a8a29e"}}>{entry.weight>0?"·":""}</span><span style={{fontSize:10,color:"#78716c"}}>{label}</span><span style={{fontSize:10,color:c.accent,fontWeight:600}}>{display}</span></>);})()}
</div>}

...
const saveManifestDay=async(wo,sd,entries)=>{
  ...
  try{
    ...
    if(newCount>0&&existingCount>0&&newCount<existingCount){
      const lost=existingCount-newCount;
      console.warn("[SAVE] BLOCKED: Refusing to overwrite "+fbKey+" — would lose "+lost+" entries ("+existingCount+" → "+newCount+"). If intentional, delete stops individually.");
      return;
    }
  }catch(readErr){

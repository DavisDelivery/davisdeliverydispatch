...
const saveDrivers=async(drvList,ts)=>{
  if(!window._fbOps)return;
  await window._fbOps.write("config/drivers",{drivers:drvList,updatedAt:ts||Date.now()});
};
...
useEffect(()=>{
  ...
  const unsubDrivers=subscribeDrivers((fbDrivers,fbTs)=>{
    ...
    if(fbDrivers&&fbDrivers.length>0){
      /* If our local timestamp is newer, we already saved — ignore this echo */
...
useEffect(()=>{
  ...
  const ts=Date.now();
  driverLocalTs.current=ts;
  localStorage.setItem("dd_drivers_ts",String(ts));
  saveDrivers(drivers,ts).then(()=>{setTimeout(()=>{dirtyDriversRef.current=false;},5000);}).catch(e=>console.error("Drv save:",e));
},[drivers]);

...
useEffect(()=>{
  ...
  const unsubManifests=subscribeManifests(wo,(fbData)=>{
    /* Don't overwrite if we just saved (within 2 seconds) */
    ...
    setLog(prev=>{
      ...
      Object.entries(fbData).forEach(([dayKey,entries])=>{
        ...
        /* If Firebase has data, use it. If Firebase is empty but local has data, keep local and save it to Firebase */
        if(entries&&entries.length>0){
          updated[lk]=entries;
          prevLogRef.current[lk]=JSON.stringify(entries);
        }else if(localEntries.length>0){
          /* Local has data Firebase doesn't — push local to Firebase */
          saveManifestDay(wo,dayIdx,localEntries).catch(e=>console.error("Push local to FB:",e));
        }
      });
      return updated;
    });
...
useEffect(()=>{
  ...
  const timer=setTimeout(()=>{
    ...
    if(saved)lastSaveTime.current=Date.now();
  },500);

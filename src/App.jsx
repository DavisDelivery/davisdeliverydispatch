...
function GoogleMapView({stops,drivers,height,onStopClick,activeDriver,activeLoad,showSearch,searchLabel,onAssignStop}){
...
<select value={entry.driverId+"-"+(entry.loadNum||1)} onChange={e=>{e.stopPropagation();const[did,ln]=e.target.value.split("-").map(Number);reassign(entry.id,did);if(ln>0)setLoadNum(entry.id,ln);}} style={{background:"#f5f5f4",border:"1px solid #e7e5e4",borderRadius:5,padding:"2px 4px",fontSize:9,color:"#57534e",cursor:"pointer",maxWidth:80}}><option value="0-1">Assign</option>{drivers.map(dd=>{const nl=getDriverLoadOptions(dd.id);return nl>1?Array.from({length:nl},(_,i)=>i+1).map(ln=><option key={dd.id+"-"+ln} value={dd.id+"-"+ln}>{dd.name.split(" ").map(w=>w[0]).join("")} L{ln}</option>):<option key={dd.id} value={dd.id+"-1"}>{dd.name}</option>;}).flat()}</select>
...
<select value={"0-1"} onChange={e=>{const[did,ln]=e.target.value.split("-").map(Number);reassign(entry.id,did);if(ln>0)setLoadNum(entry.id,ln);}} style={{background:"#f5f5f4",border:"1px solid #e7e5e4",borderRadius:5,padding:"3px 6px",fontSize:10,color:"#57534e",cursor:"pointer"}}>
<option value="0-1">Assign...</option>{drivers.map(dd=>{const nl=getDriverLoadOptions(dd.id);return nl>1?Array.from({length:nl},(_,i)=>i+1).map(ln=><option key={dd.id+"-"+ln} value={dd.id+"-"+ln}>{dd.name.split(" ").map(w=>w[0]).join("")} L{ln}</option>):<option key={dd.id} value={dd.id+"-1"}>{dd.name}</option>;}).flat()}
</select>
...
onAssignStop={mapActiveDrv?(stopId,drvId)=>{assignInOrder(stopId,mapActiveDrv,mapActiveLoad);}:null}/>
</div>);
})()}
...
addDel(selCust,puCustomName.trim(),0,drvId,{stopType:"pickup",addr:puCustomAddr.trim(),note:puNote.trim()||null,pickupFrom:puCustomName.trim(),weight});
addDel(selCust,delStop,rate,drvId,{stopType:"delivery",addr:puDelAddr.trim()||getAddr(delStop),note:(puNote.trim()?puNote.trim()+" | ":"")+(puMiles?puMiles+"mi":""),weight,fuelPct:(cd.fuel_surcharge&&!cd.fuel_included)?cd.fuel_surcharge:0,isHourly:cd.rate_type==="hourly",priority:cd.priority});

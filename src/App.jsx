...
const printDailyLog=()=>{printContent(`Daily Log`,()=>{let h=`<div class="header"><div><h1>DAVIS DELIVERY — Daily Log</h1><div class="sub">${wd[sd].name} — ${wd[sd].date}</div></div><div class="total">${fmt(dc.total)}</div></div>`;const{totalMins:shiftMins}=getShiftSummary(emDk);if(shiftMins>0){const _pLG=dl.filter(e=>e.isHourly&&e.liftgateApplied).length;const _pDist=dl.filter(e=>e.isHourly&&DISTANCE_BONUS_STOPS.includes(e.stop)).length;const _pBilled=shiftMins+(_pLG+_pDist)*60;const hrs=Math.round(_pBilled/15)*15/60;h+=`<div class="emser"><span class="lbl">⏱ Emser Hours: ${formatMins(_pBilled)}${_pLG>0?" (incl "+_pLG+"h LG)":""}${_pDist>0?" (incl "+_pDist+"h distance)":""}</span><span class="val">${fmt(102.50*hrs)}</span></div>`;}drivers.forEach((drv,di)=>{const de=drvEntries(drv.id);if(!de.length)return;h+=`<div class="section"><div class="section-title"><span>${drv.name} (${de.filter(e=>e.stopType!=="pickup").length} deliveries)</span><span class="amt">${fmt(de.filter(e=>!e.isHourly&&e.stopType!=="pickup").reduce((s,e)=>s+e.baseRate,0))}</span></div>`;h+=`<table><tr><th>#</th><th>Type</th><th>Stop</th><th>Address</th><th>Notes</th><th style="text-align:right">Rate</th></tr>`;de.forEach((e,i)=>{const tag=e.stopType==="pickup"?'<span class="tag pu">PU</span>':e.priority?'<span class="tag pri">PRI</span>':'<span class="tag del">DEL</span>';h+=`<tr><td>${i+1}</td><td>${tag}</td><td><b>${e.stop}</b><br><span style="font-size:10px;color:#78716c">${e.customer}</span></td><td style="font-size:10px;color:#78716c">${e.addr||""}</td><td>${e.instructions?'<span class="instr">📋 '+e.instructions+'</span><br>':""} ${e.note?'<span class="note">'+e.note+'</span>':""}${e.shipPlan?' <b style="color:#ea580c">SP# '+e.shipPlan+'</b>':""}</td><td style="text-align:right;font-weight:700;white-space:nowrap">${e.isHourly?"HR":fmt(e.baseRate)}</td></tr>`;});h+=`</table></div>`;});return h;});};

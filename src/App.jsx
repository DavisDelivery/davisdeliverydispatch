...
{drivers.map((d,di)=>({d,di})).filter(({d})=>d.active!==false||driverHasWeekEntries(d.id)).map(({d,di})=><div key={d.id} style={_s.flexC4}>
...
{drivers.map((d,di)=>({d,di})).filter(({d})=>d.active!==false||driverHasWeekEntries(d.id)).map(({d,di})=>{const numLoads=getDriverLoadOptions(d.id);const isActive=mapActiveDrv===d.id;const initials=d.name.split(" ").map(w=>w[0]).join("").toUpperCase();return(<Fragment key={d.id}>

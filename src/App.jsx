...
let _fbResolve;
const _fbReady=new Promise((res)=>{_fbResolve=res;});
(()=>{
...
/* Helper: subscribe to a Firestore path, handling Firebase not-yet-loaded */
const _subscribe=(setupFn)=>{
  let unsub=null;
  let cancelled=false;
  const start=()=>{
    if(cancelled||!window._fb)return;
    try{unsub=setupFn();}catch(e){console.error("[SUB] Setup failed:",e);}
  };
  if(window._fbLoaded&&window._fb)start();
  else _waitForFB().then(start);
  return()=>{cancelled=true;if(unsub)unsub();};
};

/* Wait for Firebase to be available — doesn't rely on _fbReady promise */
const _waitForFB=()=>{
  if(window._fbLoaded&&window._fb)return Promise.resolve();
  return new Promise((resolve,reject)=>{
    let attempts=0;
    const check=()=>{
      if(window._fbLoaded&&window._fb){resolve();return;}
      attempts++;
      if(attempts>100){reject(new Error("Firebase not loaded after 10 seconds"));return;}
...
const saveManifestDay=async(wo,sd,entries)=>{
  ...
  console.log("[FB-SAVE] Writing manifests/"+key,"entries:",safe.length);
  await window._fb.setDoc(window._fb.doc(_db(),"manifests",key),{entries:safe,updatedAt:Date.now()});
  console.log("[FB-SAVE] OK manifests/"+key);
};
const subscribeManifests=(wo,cb)=>{
  ...
  const start=()=>{
    if(cancelled||!window._fb)return;
    DAYNAMES_FB.forEach((abbr)=>{
      const key=`${wo}-${abbr}`;
      unsubs.push(window._fb.onSnapshot(window._fb.doc(_db(),"manifests",key),(snap)=>{
        cb({[key]:snap.exists()?(snap.data().entries||[]):[]});
      },(err)=>{
        console.error("[SUB-ERR] manifests/"+key,err.code,err.message);
      }));
    });
  };
  if(window._fbLoaded&&window._fb)start();
...
const subscribeDrivers=(cb)=>_subscribe(()=>
  window._fb.onSnapshot(window._fb.doc(_db(),"config","drivers"),snap=>{
...
const subscribeEmserHours=(cb)=>_subscribe(()=>
  window._fb.onSnapshot(window._fb.collection(_db(),"emserHours"),snap=>{
...
const subscribeEmserShifts=(cb)=>_subscribe(()=>
  window._fb.onSnapshot(window._fb.collection(_db(),"emserShifts"),snap=>{
...
const subscribeDispatchNotes=(cb)=>_subscribe(()=>
  window._fb.onSnapshot(window._fb.collection(_db(),"dispatchNotes"),snap=>{
...
const subscribeNotifications=(driverId,cb)=>_subscribe(()=>{
  const col=window._fb.collection(_db(),"notifications",String(driverId),"items");
...
const subscribeQuotes=(cb)=>_subscribe(()=>{
...
const saveMessage=async(channelKey,msg)=>{
  await _waitForFB();
  if(!_db())throw new Error("Firebase DB not available");
  console.log("[MSG] Writing to messages/"+channelKey+"/items",msg.text?.slice(0,30));
  await window._fb.addDoc(window._fb.collection(_db(),"messages",channelKey,"items"),{...msg,timestamp:Date.now()});
  console.log("[MSG] Write OK");
};
const subscribeMessages=(channelKey,cb)=>_subscribe(()=>{
  ...
  return window._fb.onSnapshot(col,(snap)=>{
    ...
    console.error("[MSG-SUB] Error on channel",channelKey,err);
  });
});
...
const saveDriverLocation=async(driverId,lat,lng)=>{
  await _waitForFB();
  if(!_db())return;
  await window._fb.setDoc(window._fb.doc(_db(),"driverLocations",String(driverId)),{lat,lng,updatedAt:Date.now()});
};
const subscribeDriverLocations=(cb)=>_subscribe(()=>
  window._fb.onSnapshot(window._fb.collection(_db(),"driverLocations"),snap=>{
    const locs={};snap.forEach(d=>{locs[d.id]=d.data();});cb(locs);
  })
);
...
const saveInvoice=async(invoice)=>{
  await _waitForFB();
  if(!_db())return;
  await window._fb.setDoc(window._fb.doc(_db(),"invoices",String(invoice.id)),{...invoice,updatedAt:Date.now()});
};
const subscribeInvoices=(cb)=>_subscribe(()=>
  window._fb.onSnapshot(window._fb.collection(_db(),"invoices"),snap=>{
...
const subscribeCustomStops=(cb)=>_subscribe(()=>
  window._fb.onSnapshot(window._fb.doc(_db(),"config","customStops"),snap=>{
...
useEffect(()=>{
  ...
  _waitForFB().then(async()=>{
    firebaseReady.current=true;
    setFbConnected(true);
    console.log("[FB] Firebase SDK loaded");
    try{
      /* Test write */
      await window._fb.setDoc(window._fb.doc(_db(),"_test","ping"),{t:Date.now()});
      console.log("[FB] ✓ Firestore write test PASSED");
      /* Test read */
      const snap=await window._fb.getDoc(window._fb.doc(_db(),"_test","ping"));
      if(snap.exists())console.log("[FB] ✓ Firestore read test PASSED");
      else console.warn("[FB] ⚠ Read returned empty");
    }catch(e){
      console.error("[FB] ✗ Firestore test FAILED:",e.code,e.message);
      setSaveStatus("⚠ DB error: "+e.code);
      showToast("⚠ Firebase error: "+(e.code||e.message)+". Check Firestore rules.");
    }
  });
...
<button onClick={()=>deleteDel(entry.id)} style={{background:"none",border:"none",color:"#dc2626",fontSize:10,cursor:"pointer",padding:"2px 0 0",opacity:0.7}}>Delete</button>
</div>
</div>
...
<button onClick={()=>deleteDel(entry.id)} style={{background:"none",border:"none",color:"#dc2626",fontSize:11,cursor:"pointer",padding:"2px 0 0",opacity:0.7}}>Delete</button>
</div>
</div>
{isImetco&&<div style={{marginTop:8,display:"flex",alignItems:"center",gap:6}}>
<span style={{fontSize:11,fontWeight:700,color:"#ea580c",flexShrink:0}}>Ship Plan #:</span>
  ...
  const unsubManifests=subscribeManifests(wo,(fbData)=>{
    ...
    Object.entries(fbData).forEach(([dayKey,ents])=>{
      ...
      if(dayIdx>=0){
        ...
        console.log("[DRV-SUB] Received",dayKey,"entries:",ents.length);
      }
    });

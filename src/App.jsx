...
const saveManifestDay=async(wo,sd,entries)=>{
  ...
  try{
    ...
    const merged=entries.map(e=>{
      ...
      if(!fbE)return e;
      const out={...e};
      if(fbE.status&&fbE.status!==e.status)out.status=fbE.status;
      if(fbE.arrivedAt&&!e.arrivedAt)out.arrivedAt=fbE.arrivedAt;
      if(fbE.departedAt&&!e.departedAt)out.departedAt=fbE.departedAt;
      ...
      Object.entries(fbData).forEach(([fbKey,payload])=>{
        ...
        if(dirtyOrCooldown){
          ...
          finalEntries=localEnts.map(localE=>{
            ...
            const out={...localE};
            if(fbE.status)out.status=fbE.status;
            if(fbE.arrivedAt)out.arrivedAt=fbE.arrivedAt;
            if(fbE.departedAt)out.departedAt=fbE.departedAt;
...
{!showAddCustomDel?<button onClick={()=>{setShowAddCustomDel(true);setCustomDelName("");setCustomDelAddr("");setCustomDelRate("");setCustomDelNote("");setCustomDelMiles("");setCustomDelLiftgate(false);setCustomDelGravel(false);setCustomDelExtraPallets(false);setCustomDelNoFuel(false);setCustomDelCalcLoading(false);}} style={{display:"block",width:"100%",marginTop:8,background:"#fafaf9",border:"2px dashed #d6d3d1",borderRadius:12,padding:"14px 16px",cursor:"pointer",textAlign:"center",color:"#78716c",fontSize:13,fontWeight:600}}>+ Add New Delivery Location</button>
...
setShowAddCustomDel(false);setCustomDelName("");setCustomDelAddr("");setCustomDelRate("");setCustomDelNote("");setCustomDelPermanent(true);setCustomDelNoFuel(false);setCustomDelLiftgate(false);setCustomDelGravel(false);setCustomDelExtraPallets(false);setCustomDelMiles("");

...
const addQuote=drvId=>{const stopName=customStop||customerName||(isOneOff?"One-Off":"Quote");const note=[`${mi}mi`,liftgate?"LG":"Fuel",gravel?"Gravel":"",extraPallets?"4-5 Pallets":""].filter(Boolean).join(" | ");onAdd(isOneOff?"One-Off Delivery":"Quote Delivery",stopName,total,drvId,{note,addr:destAddr,stopType:"delivery"});};
return(
<div>
...
<div style={_s.flexBtwMb10}><span style={{fontSize:13,fontWeight:700}}>Distance</span><button onClick={()=>setShowApiInput(!showApiInput)} style={{background:"#f5f5f4",border:"none",borderRadius:6,padding:"4px 8px",fontSize:10,color:"#78716c",cursor:"pointer"}}>{apiKey?"API Key Set":"Set API Key"}</button></div>
...
useEffect(()=>{
  ...
  let watchId=null;
  const sendPos=(pos)=>{
    driverPosRef.current={lat:pos.coords.latitude,lng:pos.coords.longitude};
...
if(driverPosRef.current){doCalc(new window.google.maps.LatLng(driverPosRef.current.lat,driverPosRef.current.lng));}
else{const curAddr=entry.addr||getAddr(entry.stop);if(curAddr){doCalc(curAddr);}else{setEtaD(entry.id,"?",dest);showToast("No GPS & no current address");}}
}}
style={{flex:1,border:"1px solid #d6d3d1",borderRadius:8,padding:"8px",fontSize:12,outline:"none",background:"#fff",color:entry.etaDest?"#1c1917":"#a8a29e"}}>
<option value="">ETA to where?</option>
{entries.filter((_,ei)=>ei>i&&_.status!=="departed").map(ne=><option key={ne.id} value={ne.stop}>{ne.stop}</option>)}

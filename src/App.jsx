...
const subscribeDriverLocations=(cb)=>{
  const run=()=>{
    return window._fb.onSnapshot(window._fb.collection(_db(),"driverLocations"),snap=>{
...
const subscribeInvoices=(cb)=>{
  const run=()=>{
    return window._fb.onSnapshot(window._fb.collection(_db(),"invoices"),snap=>{
...
/* setLog wrapper — marks changed days as dirty so Firebase doesn't overwrite them */
...
/* ── FIREBASE SYNC ── */
useEffect(()=>{
  /* Wait for Firebase to actually load before marking ready */
  _fbReady.then(()=>{
    firebaseReady.current=true;
    setFbConnected(true);
    console.log("[FB] Firebase SDK loaded and connected");
  });
  ...
  const timer=setTimeout(()=>{
    ...
    toSave.forEach(dayKey=>{
      ...
      if(!isNaN(weekOff)&&dayIdx>=0&&dayIdx<=4){
        ...
        saveManifestDay(weekOff,dayIdx,entries).then(()=>{
          console.log("[SAVE] OK",dayKey);
          prevLogRef.current[dayKey]=JSON.stringify(entries);
...
<div style={{display:"flex",alignItems:"center",gap:8}}>
<a href={window.location.pathname} onClick={e=>{e.preventDefault();setView("manifest");setSelCust(null);setQuoteMode(null);window.scrollTo(0,0);}} style={{cursor:"pointer",display:"flex",alignItems:"center",gap:8}}>
...
<div style={{display:"flex",alignItems:"center",gap:10}}>
{view!=="manifest"&&<button onClick={()=>{setView("manifest");setSelCust(null);setQuoteMode(null);window.history.replaceState(null,"",window.location.pathname);}} style={{background:BRAND.main,border:"none",color:"#fff",borderRadius:8,padding:"7px 16px",cursor:"pointer",fontSize:12,fontWeight:700}}>{"← Dashboard"}</button>}
...
<button onClick={()=>{setView("manifest");window.history.replaceState(null,"",window.location.pathname);}} style={{background:BRAND.main,border:"none",color:"#fff",borderRadius:8,padding:"7px 16px",cursor:"pointer",fontSize:12,fontWeight:600}}>{"← Dashboard"}</button>
<div style={{display:"flex",gap:6}}>
...
<div style={{display:"flex",alignItems:"center",gap:6}}>
<a href={window.location.pathname} onClick={e=>{e.preventDefault();setView("manifest");setSelCust(null);setQuoteMode(null);window.scrollTo(0,0);}} style={{cursor:"pointer"}}><img src={LOGO_WHITE} alt="Davis Delivery" style={{height:28,objectFit:"contain"}}/></a>
<div>
<div style={{fontSize:8,color:"#93c5fd",fontWeight:600,opacity:0.7}}>v{APP_VERSION}</div>
<div style={{display:"flex",alignItems:"center",gap:3}}><span style={{display:"inline-block",width:5,height:5,borderRadius:3,background:fbConnected?"#16a34a":"#dc2626"}}/><span style={{fontSize:7,color:fbConnected?"#6ee7b7":"#fca5a5"}}>{fbConnected?"synced":"offline"}</span></div>
</div>

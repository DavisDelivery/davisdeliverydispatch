...
const[fbDebug,setFbDebug]=useState(""); /* visible debug messages */
const driverChangeSource=useRef("init"); /* "init" | "firebase" | "user" */

/* Manual test save — call from UI button */
const testFirebaseSave=async()=>{
  setFbDebug("Testing...");
  try{
    if(!window._fbOps){setFbDebug("✗ _fbOps not loaded");return;}
    setFbDebug("_fbOps exists. Writing...");
    const t=Date.now();
    await window._fbOps.write("_test/ping",{t,msg:"test from v"+APP_VERSION});
    setFbDebug("✓ Write OK! Reading back...");
    const data=await window._fbOps.read("_test/ping");
    if(data&&data.t===t){
      setFbDebug("✓ PASSED! Firebase works. t="+t);
      setSaveStatus("✓ FB works");
    }else{
      setFbDebug("⚠ Write OK but read mismatch: "+JSON.stringify(data));
    }
  }catch(e){
    setFbDebug("✗ FAILED: "+(e.code||"")+" "+(e.message||e));
  }
};
...
useEffect(()=>{
  _whenFB(async()=>{
    ...
    setFbConnected(true);
    setFbDebug("FB loaded. Testing save...");
    try{
      await window._fbOps.write("_test/ping",{t:Date.now()});
      setFbDebug("✓ Auto-test PASSED");
      setTimeout(()=>setFbDebug(""),5000);
    }catch(e){
      setFbDebug("✗ Auto-test FAILED: "+(e.code||"")+" "+(e.message||e));
...
useEffect(()=>{
  ...
  /* Skip if same as what Firebase sent */
...
useEffect(()=>{
  if(!firebaseReady.current)return;
  const val=dispNotes[dk];
  if(val===undefined)return;
  if(prevNotesRef.current[dk]===val)return;
  prevNotesRef.current[dk]=val;
  const timer=setTimeout(()=>{
    saveDispatchNote(dk,val||"").catch(e=>console.error("Note save:",e));
  },500);
  return()=>clearTimeout(timer);
},[dispNotes,dk]);
...
useEffect(()=>{
  ...
  if(prevShiftsRef.current[dk]===json)return;
  prevShiftsRef.current[dk]=json;
  const timer=setTimeout(()=>{
...
{/* Firebase debug bar */}
{fbDebug&&<div style={{background:"#292524",padding:"6px 20px",fontSize:10,fontFamily:"monospace",color:fbDebug.includes("✓")?"#4ade80":fbDebug.includes("✗")?"#fca5a5":"#fde68a",borderBottom:"1px solid #44403c"}}>{fbDebug}</div>}
<div style={{background:"#1c1917",padding:"4px 20px",borderBottom:"1px solid #44403c"}}>
<button onClick={testFirebaseSave} style={{background:"#d97706",border:"none",color:"#fff",borderRadius:6,padding:"4px 12px",cursor:"pointer",fontSize:10,fontWeight:700}}>🧪 Test Save</button>
<span style={{fontSize:9,color:"#78716c",marginLeft:8}}>Tap to test if Firebase saves work</span>
</div>
<div style={{display:"flex",gap:8,alignItems:"center"}}>

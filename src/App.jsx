    ...
    _rawSetLog(prev=>{
      ...
      Object.entries(fbData).forEach(([fbKey,payload])=>{
        ...
        if(dirtyOrCooldown){
          ...
          const out=fbFiltered.map(fbE=>{
            const localE=localById[fbE.id];
            if(!localE)return fbE;
            return _mergeEntryDispatcher(localE,fbE);
          });
          /* Append local-only entries (preserves dispatcher's in-flight
             additions) — but NOT tombstoned ones. Order: FB order first,
             local-only at end. */
          localEnts.forEach(localE=>{
            if(localE&&localE.id&&!fbById[localE.id]&&!tombSet.has(localE.id))out.push(localE);
          });
          finalEntries=out;
        }
...
const addPhoto=(eid,dataUrl)=>{
  ...
  /* Compress before upload. Raw iPhone camera output is 4-6 MB per shot;
     compressed to 1200px @ 0.6 quality it's typically ~200 KB. The full-size
     upload over cellular was the smoking gun for 'photos won't come through' —
     a slow connection + several large files in flight + iOS Safari's
     tendency to silently drop big requests = upload timeouts that the user
     experienced as the camera button doing nothing.
     1200px is plenty for proof-of-delivery shots without making a 5MB
     upload over a truck's LTE. */
  const startUpload=(small)=>{
    showToast("📤 Uploading photo…");
    const tryUpload=(attempt)=>fetch(small).then(r=>r.blob()).then(b=>{
  ...
  compressPhoto(dataUrl,0.6,1200).then(startUpload).catch(()=>{
    /* Compression failed — try the raw photo as a fallback. */
    console.warn("[PHOTO-DRV] Compression failed, uploading raw");
    startUpload(dataUrl);
  });
};
